(function ( $ ) {
			...
			lightBox.listen('afterChange', function() {
				...
				if ( $iframe.length ) {
					...
					if ( -1 !== src.indexOf( 'autoplay' ) ) {
						$iframe[0].src = src.replace( 'autoplay=0', 'autoplay=1' );
					} else {
	...
	sober.productGallerySlider = function() {
		...
		// Play video when slide changed
		$carousel.on( 'translated.owl.carousel', function(event, a) {
			var $currentSlide = $( '.owl-item.active', $( this ) ),
				$iframe = $( 'iframe', $currentSlide );
